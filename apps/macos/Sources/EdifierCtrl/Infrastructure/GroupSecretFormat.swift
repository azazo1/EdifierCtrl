...
/// 独立管理组名文件格式, 未知版本与无效文件均保留原文供用户处理.
enum GroupSecretFormat {
    ...
    static func decode(_ data: Data) throws -> GroupSecretDocument {
        do {
            ...
            // 解码器的原始描述可能包含组名文本, 只向 UI 和日志传递固定错误.
    ...
    enum FormatError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case let .unsupportedVersion(version): return "不支持组名文件版本 \(version), 原文件已保留."
            case .invalidData: return "数据目录中的组名文件格式无效, 原文件已保留."
            }
        }
    }
}
